...
struct GameView: View {
    ...
    private var headerView: some View {
        HStack {
            ...
            // Changed from showing static duration to remaining time
            if gameViewModel.settings.gameMode == .timeAttack || gameViewModel.timeRemaining > 0 {
                Text(timeRemainingFormatted)
                    .font(.title)
                    .monospacedDigit()  // Ensures numbers don't shift width
...
#Preview {
    GameView(gameViewModel: MemoryGame(settings: GameSettings()))
}
